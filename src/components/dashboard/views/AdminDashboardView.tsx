...
export default function AdminDashboardView() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Control Center — Staff management and analytics."
    >
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {staff.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="py-8 text-center text-slate-500">No staff yet.</td>
                    </tr>
                  ) : (
                    staff.map((s: StaffMember) => (
                      <tr key={s._id} className="border-b border-slate-100 dark:border-slate-800">
                        ...
                        <td className="py-3">
                          <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${s.role === "doctor" ? "bg-teal-100 text-teal-700 dark:bg-teal-900/50 dark:text-teal-300" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300"}`}>
                            {s.role}
                          </span>
                        </td>
                      ...
                      >
                        ...
                        <td className="py-3 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => deleteAppointmentMutation.mutate(apt._id)}
      ...
      <AddStaffModal isOpen={doctorModalOpen} onClose={() => { setDoctorModalOpen(false); setModalError(null); }} role="doctor" onSubmit={(email, password) => addDoctorMutation.mutateAsync({ email, password })} error={modalError} onClearError={() => setModalError(null)} />
      <AddStaffModal isOpen={receptionistModalOpen} onClose={() => { setReceptionistModalOpen(false); setModalError(null); }} role="receptionist" onSubmit={(email, password) => addReceptionistMutation.mutateAsync({ email, password })} error={modalError} onClearError={() => setModalError(null)} />
    </DashboardLayout>
  );
}
